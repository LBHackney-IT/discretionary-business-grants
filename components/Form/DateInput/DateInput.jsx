...
const DateInput = ({ label, inputRef, error, value, onChange }) => {
  ...
  useEffect(() => {
    ...
    day !== '' && month !== '' && year !== '' && onChange(`${year}-${month}-${day}`);
  }, [date]);
  return (
    <div
    ...
    >
      <fieldset className="govuk-fieldset" role="group" aria-describedby={`${name}-hint`}>
        ...
        <div className="govuk-date-input" id={name}>
          <div className="govuk-date-input__item">
            <div className="govuk-form-group">
              ...
              <input
                className={cx('govuk-input govuk-date-input__input govuk-input--width-2', {
                ...
                inputMode="numeric"
                value={date.day}
                onChange={({ target: { value } }) => setDate({ ...date, day: value })}
                ref={inputRef}
              />
            </div>
          </div>
          <div className="govuk-date-input__item">
            <div className="govuk-form-group">
              ...
              <input
                ...
                inputMode="numeric"
                value={date.month}
                onChange={({ target: { value } }) => setDate({ ...date, month: value })}
              />
            </div>
          </div>
          <div className="govuk-date-input__item">
            <div className="govuk-form-group">
              ...
              <input
                className={cx('govuk-input govuk-date-input__input govuk-input--width-4', {
                  'govuk-input--error': error
                })}
                id={`${name}-year`}
                name={`${name}-year`}
                ...
                inputMode="numeric"
                value={date.year}
                onChange={({ target: { value } }) => setDate({ ...date, year: value })}
              />
            </div>
          </div>
        </div>
