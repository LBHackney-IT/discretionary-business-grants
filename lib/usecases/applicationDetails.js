...
export default async ({ clientGeneratedId }) => {
  ...
  const documents = await getDocuments(dbInstance, clientGeneratedId);
  ...
  return {
    application: {
      ...
      contact: {
        contactTypeId: CONTACT_TYPE[application.contact_type_id],
      ...
      business: {
        companyStructureId: COMPANY_STRUCTURE[application.company_structure_id],
        ...
        siteDescriptionId: SITE_DESCRIPTION[application.site_description_id],
      ...
      declaration: {
        stateAidOptionId: STATE_AID_OPTION[application.state_aid_option_id],
