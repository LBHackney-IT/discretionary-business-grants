...
const listApplications = ({ getDbInstance }) => async ({
  ...
  const grantOfficerFilter = await calculateGrantOfficersFilter(grantOfficer);
  ...
  async function calculateGrantOfficersFilter(requiredOfficer) {
    const data = await fetchGrantOfficers('http://localhost:3000');
    let grantOfficers = data.grantOfficers;
    const allKnownGrantOfficers = grantOfficers.map(
