...
describe('listApplications', () => {
  ...
  test('when status is undefined all known states are included in the query', async () => {
    const { databaseSpy, container } = createContainerAndDatabaseSpy(0);
  ...
  test('business type ids are included in the query', async () => {
    ...
    expect(databaseSpy).toHaveBeenCalledWith(
      expect.stringContaining(
        'b.site_description_id IN ($(businessTypeFilter:list))'
      ),
      expect.anything()
    );
  });

  test('when businessType is undefined a list of known businessTypes are included in the query', async () => {
    const { databaseSpy, container } = createContainerAndDatabaseSpy(0);
