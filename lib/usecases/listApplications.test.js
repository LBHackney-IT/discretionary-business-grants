...
describe('listApplications', () => {
  ...
  test('when grantOfficer is undefined a list of known grantOfficer are included in the query', async () => {
    const { databaseSpy, container } = createContainerAndDatabaseSpy(0);
    const grantOfficersResponse = { data: { grantOfficers: [] } };
    const expectedGrantOfficers = [];
    for (let x = 1; x <= 3; x++) {
      const identifier = `${faker.name.firstName()} ${faker.name.lastName()} <${
        faker.internet.email
      }>`;
      expectedGrantOfficers.push(identifier);
      grantOfficersResponse.data.grantOfficers.push({ identifier });
    }
    const axiosMockAdapter = new AxiosMockAdapter(axios);
    // axiosMockAdapter
    //   .onGet('/api/grant-officers')
  ...
  test.skip('when grantOfficer is defined & found, only that grantOfficer is queried', async () => {
    const { databaseSpy, container } = createContainerAndDatabaseSpy(0);
    const typeIndex = 0;
    const desiredType = TYPE_OF_BUSINESS[typeIndex];
    const expectedType = typeIndex + 1;
