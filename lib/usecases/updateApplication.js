...
export default async ({ clientGeneratedId, data, user }) => {
  ...
  if (data.status && data.validations) {
    return createResponse('Only one of status or validations is permitted');
  }
  ...
  async function processValidations(validations){
    ...
    await addHistoryEntry(dbInstance, grantApplicationId, user, 'Validations updated.');
  ...
  async function processStatus(statusString){
    ...
    await addHistoryEntry(dbInstance, grantApplicationId, user, `Status updated to "${data.status}"`);
