...
async function meetsEligibilityCriteria({
  ...
  return (
    ...
    options.TYPE_OF_BUSINESS.includes(typeOfBusinessId) &&
    ...
    options.VALID_RETEABLE_LIMIT_ANSWER.includes(rateableLimitAnswerId) &&
    options.VALID_BUSINESS_SIZE.includes(businessSizeId)
  );
}
...
async function isValidContact({
  ...
  return (
    ...
    options.CONTACT_TYPE.includes(contactTypeId)
  );
}
...
async function isValidBusiness({
  ...
  return (
    options.COMPANY_STRUCTURE.includes(companyStructureId) &&
    parseInt(fullTimeEmployees) >= 1 &&
    parseInt(percentageFallInIncome) >= 1 &&
    parseInt(percentageFallInIncome) <= 100 &&
    options.SITE_DESCRIPTION.includes(siteDescriptionId)
  );
}
